...
CONSTRAINT ROUTE_drfk_1
  ...
  -- TODO check the relational algebra i.e. make sure text size is correct,
  -- some text is smaller than others on purpose and make sure the correct
  -- symbols are used and in the right place
...
-- TODO retake the screenshot for this query (you cocked up the column names)
SELECT pub_ID, pub_name, pub_price, PUBLISHER_publisher_ID
FROM PUBLICATION
...
πID, name, price, publisherID(σprice < 3.00(PUBLICATION))
--note order by is not possible in relational algebra


-- TODO retake this screenshot as the join has been changed
...
OUTLET ⨝ outletOutID = ordtableOutID ORDTABLE
-- TODO check this symbol works in word
...
--are taking. Another driver can also use this just by changing the id.
πrouteID, routeName(σdriverID = 2(ROUTE))


-- TODO take a screenshot of this and replace with the UPDATE query
SELECT DISTINCT COUNT(pub_name)
FROM PUBLICATION;
--An outlet may wish to know how many different publications there are. This
--query counts all the unique items in the publication table.
count(name)(πname(PUBLICATION))
...
-- TODO take another screenshot of this when using a left join, test it again,
-- not sure if there is a difference between left outer and normal join
...
FROM VEHICLE
GROUP BY veh_make;
--JM may want to know what cars they own and how many of each make, this query
--will output the make along with the amount they own.


SELECT AVG (ret_noOfParcels) average
FROM RETURNITEM
HAVING COUNT(*) > 2;
--An outlet may wish to find out the average amount of parcels that are returned
--so that they can adjust the amount they buy in their next order, thus saving money.
--This query outputs the average number of parcels returned.
